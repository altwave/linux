...
{
	printk(KERN_INFO "Called bpf_hello_is_valid_access at off=%d\n", off);
...
{
	printk(KERN_INFO "bpf_hello_btf_struct access called for off %d\n", off);
		...
		const void *udata)
{
	...
	case offsetof(struct hello_struct, name):
		if (bpf_obj_name_cpy(khmm_policy->name, uhmm_policy->name, sizeof(khmm_policy->name)) <= 0) {
			printk(KERN_INFO "Cannot init hello->name, khmm_policy->name=%s, uhmm_policy->name=%s, moff=%d\n", 
					khmm_policy->name, uhmm_policy->name, moff);
			return -EINVAL;
		}
